    ...
    public static Map<Integer, List<Message>> generateMessages(int maxUserId) {
        ...
        for (int i = 0; i < maxUserId; i++, k++) {
            ...
            for (int j = 0; j < numMessages; j++, k++) {
                ...
                Message message = new Message(k, text, i, System.currentTimeMillis(), random.nextBoolean() ? State.READ.INSTANCE : new State.DELETED(i));
    ...
    public static List<Entity> generateEntity() {
        ...
        for (int i = 0; i < garbage; i++) {
            switch (random.nextInt(5)) {
                case 0 -> combined.add(new Message(null, texts[random.nextInt(texts.length - 1)], -1, -1L, State.READ.INSTANCE));
                case 1 -> combined.add(new Message(-1, null, -1, -1L, State.READ.INSTANCE));
                case 2 -> combined.add(new Message(-1, texts[random.nextInt(texts.length - 1)], null, -1L, State.READ.INSTANCE));
                case 3 -> combined.add(new Message(-1, texts[random.nextInt(texts.length - 1)], -1, -1L, null));
                default -> combined.add(new Message(-1, texts[random.nextInt(texts.length - 1)], -1, null, State.READ.INSTANCE));
            }
        }
        garbage = random.nextInt(50);
        for (int i = 0; i < garbage; i++) {
